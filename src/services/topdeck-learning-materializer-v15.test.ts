...
function card(name: string, typeLine: string, cmc: number, oracleText = ''): ScryfallCard {
  return {
    id: `id-${name}`, oracle_id: `oracle-${name}`, name, lang: 'en', released_at: '2024-01-01',
    mana_cost: cmc > 0 ? `{${cmc}}` : '', cmc, type_line: typeLine, oracle_text: oracleText,
    color_identity: [], keywords: [], legalities: { commander: 'legal' }, set: 'tst', set_name: 'Test Set',
    collector_number: '1', rarity: 'common', scryfall_uri: `https://scryfall.com/card/tst/1/${encodeURIComponent(name)}`,
  };
}

function structuralDeck(prefix: string, cheapInteraction: number, threatManaValue: number): { decklist: string; cards: ScryfallCard[]; commanderName: string } {
  ...
  return {
    commanderName,
    decklist: ['// COMMANDER', `1 ${commanderName}`, '', '// MAIN', `34 ${landName}`, `${cheapInteraction} ${answerName}`, `${threatCount} ${threatName}`].join('\n'),
    cards: [
...
function snapshot(fixture: ReturnType<typeof structuralDeck>, availableAt: string) {
  return extractDeckFeatureSnapshotV15(fixture.decklist, fixture.cards, { availableAt, cardDataObservedAt: '2024-12-31T00:00:00.000Z' });
}

function candidate(fixture: ReturnType<typeof structuralDeck>, options: { playerId: string; standing: number; eventId?: string }): TopDeckLearningCandidateV15 {
  const eventId = options.eventId ?? 'test-open-2026';
  return {
    sourceId: 'topdeck', providerEventId: eventId, providerPlayerId: options.playerId,
    providerRecordId: `${eventId}:standing:${options.playerId}`, sourceUrl: `https://topdeck.gg/event/${eventId}`,
    outcomeOccurredAt: '2026-01-10T00:00:00.000Z', standing: options.standing, fieldSize: 32, topCutSize: 8,
    decklist: fixture.decklist, commanderNames: [fixture.commanderName],
    metadata: {
      provider: 'topdeck-v2', tournamentName: 'Test Open', wins: null, draws: null, losses: null,
...
function linkage(playerId: string) {
  return {
    canonicalOutcomeId: `test-open-2026:entrant:${playerId}`, independenceKey: 'test-open-2026', leakageKey: 'test-open-2026',
...
test('same historical deck materializes identical predictors even when TopDeck outcome labels differ', () => {
  ...
  const winner = materializeTopDeckLearningCandidateV15(candidate(targetFixture, { playerId: 'winner', standing: 1 }), targetSnapshot, normalizer, linkage('winner'));
  const miss = materializeTopDeckLearningCandidateV15(candidate(targetFixture, { playerId: 'miss', standing: 20 }), targetSnapshot, normalizer, linkage('miss'));
...
test('materializer rejects a feature snapshot created after the tournament outcome', () => {
  ...
  assert.throws(
    () => materializeTopDeckLearningCandidateV15(candidate(targetFixture, { playerId: 'late', standing: 4 }), lateSnapshot, normalizer, linkage('late')),
...
test('materializer rejects deck fingerprint and commander identity mismatches', () => {
  ...
  assert.throws(
    () => materializeTopDeckLearningCandidateV15(candidate(candidateFixture, { playerId: 'fingerprint', standing: 2 }), otherSnapshot, normalizer, linkage('fingerprint')),
    /fingerprint/i,
  );
  ...
  assert.throws(
    () => materializeTopDeckLearningCandidateV15(wrongCommanderCandidate, matchingSnapshot, normalizer, linkage('commander')),
    /commander/i,
  );
});
