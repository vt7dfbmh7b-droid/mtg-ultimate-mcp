...
/** Install verified snapshot cards at the shared HTTP boundary, including paged discovery. */
export function installRetainedScryfallCardDataV15(cards: ScryfallCard[]): void {
  installRetainedCardIndexV15(cards);
  searchCache.clear(); printingsCache.clear(); identifierCache.clear();
  setCache = null; setCacheAt = 0;
}

export function clearRetainedScryfallCardDataV15(): void {
  clearRetainedCardIndexV15();
  searchCache.clear(); printingsCache.clear(); identifierCache.clear();
  setCache = null; setCacheAt = 0;
}

export { isRetainedScryfallCardDataInstalledV15 } from './retained-scryfall-provider-v15.js';
...
async function scryfallRequest<T>(url: string, init: RequestInit = {}): Promise<T> {
  if (isRetainedScryfallCardDataInstalledV15()) return fetchJson<T>(url, init);
...
export function inferCardRoles(card: ScryfallCard): string[] {
  ...
  const teamWideUntapPump = /\b(?:creatures|permanents) (?:you control|target player controls)\b[\s\S]{0,240}\bget \+\d+\/\+\d+[\s\S]{0,240}\buntap\b/.test(text) || /\buntap\b[\s\S]{0,240}\b(?:creatures|permanents) (?:you control|target player controls)\b[\s\S]{0,240}\bget \+\d+\/\+\d+/.test(text);
