...
- The FF32 audit/state reconciliation reproduced the same ordering characteristic: project-state integrity run `33815556594` passed and persisted self-consistent metadata at head `e59ef9d...`, while CI run `33815556598` checked the pre-persistence `fa008f0...` tree and failed only its validation-index freshness step. A documentation-only post-persistence event is used to validate the persisted head; this is an operational race, not a source or runtime defect.
