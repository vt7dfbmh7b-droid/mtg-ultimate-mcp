...
5. Keep bounded shutdown covered by lifecycle tests and add deployment-level abort telemetry before altering the stable runtime entry point.
...
8. Resolve or close temporary validation surfaces #30 and #32 after current-source evidence capture; keep the legacy recovery surface #2 separate from PR #29 and decide its archival path.
9. At BENCH-01/INTEL-03, decide which test-only research libraries receive a product surface and which should be archived with their evidence.
