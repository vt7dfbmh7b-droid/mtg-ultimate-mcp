...
- Reason: BENCH-01 remains active. The corrected Batch D candidate-breadth result is durably persisted: Quick Draw and Virtue and Valor were unchanged at breadth 6, while Explorers of the Deep improved from 4 swaps/Bracket 2 to 9 swaps/Bracket 3. One positive fixture is insufficient for a product change, so cross-family breadth generalization is the current evidence gate.
...
The formal accepted development checkpoint remains 77a5383.... The latest fully validated executable Commander product remains e17b0a1cba659b229fd6f0b6e2df79c5e464a616. BENCH-01 Batch D and its candidate-breadth diagnostic both use that unchanged frozen product source. The candidate-breadth diagnostic changed only candidatePackagesPerRound from 4 to the supported maximum 6: Quick Draw remained 8 swaps/Bracket 3, Virtue and Valor remained 4 swaps/Bracket 2, while Explorers of the Deep improved from 4 swaps/Bracket 2 to 9 swaps/Bracket 3. This proves bounded package-diversification sensitivity for Explorers, but not a repeated unrelated-fixture pattern. No Commander product repair is authorized yet. The downstream compound-component preservation gate remains required and must not be weakened.
...
- Active branch status: **bench01-candidate-breadth-generalization-active**
...
- Keep the latest fully validated executable Commander baseline e17b0a1cba659b229fd6f0b6e2df79c5e464a616 frozen while BENCH-01 breadth-generalization evidence is collected.
- Treat the corrected Batch D candidate-breadth persistence gate as closed: the tracked Git blob is non-empty and records Quick Draw unchanged, Virtue and Valor unchanged, and Explorers of the Deep improved at breadth 6.
- Require repeated breadth sensitivity across an unrelated Commander family before authorizing any generic candidate-discovery/ranking repair; one positive Explorers fixture alone is insufficient.
...
- Candidate breadth sensitivity in one benchmark fixture is evidence of bounded-search sensitivity, not proof of a generic product defect; require unrelated cross-fixture reproduction before changing discovery/ranking behavior.
