...
| target-aware candidate-generation tests | failed B5 gates drive candidate lanes before aspirational role deficits | average-nonland-mv additions only create positive curve reductions; the active curve lane can inspect bounded non-positive-pressure cuts; lower brackets unchanged | candidates can address the evaluator's real blocker and use the smallest safe reduction that crosses the threshold | deterministic regressions green; `efcffc2...` exposed KF-018 at 2.61, fresh live repair pending |
| strategy-preservation / cut-impact tests | autonomous target repair does not silently erase a substantive commander strategy or freeze on weak incidental overlap | per-swap cut roles and affinity; aggregate package audit; missing evidence and uncompensated meaningful loss fail closed; meaningful status requires substantive command-zone evidence | Aurelia-style combat affinity stays protected, weak four-point Najeela `big-mana` overlap does not block the safer Vanquish curve cut, and strategically matched replacement remains legal | deterministic regressions green on the current working lineage; first `7fcd3ca...` live run exposed KF-016; fresh Marvel revalidation pending |
