...
const TESTS:Test[]=[
 {id:'rider_to_stroke',cut:'Murderous Rider',add:C.stroke,manualClass:'broader MV3 instant removal; Adventure baseline already qualifies Ystola'},
 ...
 {id:'scythe_to_denial',cut:"Reaper's Scythe",add:C.denial,manualClass:'lose one Ystola trigger; gain flexible two-mana counter with replacement draw'},
 ...
 {id:'scythe_to_rift',cut:"Reaper's Scythe",add:C.rift,manualClass:'lose one Ystola trigger; gain premier bounce/overload reset'},
 {id:'scythe_to_cryptic',cut:"Reaper's Scythe",add:C.cryptic,manualClass:'retain Ystola trigger; gain high-flexibility MV4 instant'},
 {id:'scythe_to_brainstorm',cut:"Reaper's Scythe",add:C.brainstorm,manualClass:'lose trigger; gain one-mana selection/combo smoothing'},
 ...
 {id:'tome_to_denial',cut:'Tome of Legends',add:C.denial,manualClass:'trade slow draw for counter that later replaces cards'},
 ...
 {id:'final_judgment_to_damn',cut:'Final Judgment',add:C.damn,manualClass:'trade exile wipe/Ystola trigger for much cheaper flexible wipe'},
...
function faceQualifyingOpportunities(c:ScryfallCard|undefined):number{if(!c)return 0;const faces=c.card_faces??[];if(faces.length){return faces.filter(f=>{const t=(f.type_line??'').toLowerCase();const faceMv=(f.mana_cost??'').match(/\{(\d+)\}/)?.[1];const approx=faceMv?Number(faceMv):c.cmc;return !t.includes('creature')&&!t.includes('land')&&approx>=3;}).length;}const t=c.type_line.toLowerCase();return !t.includes('creature')&&!t.includes('land')&&c.cmc>=3?1:0;}
function ystolaOfferCount(d:Deck){let count=0;for(const e of d.parsed.main){const c=cardByName(d,e.name);if(c)count+=Math.min(1,faceQualifyingOpportunities(c))*e.quantity;}return count;}
...
async function main(){
 ...
 for(const t of TESTS){const d=await resolve(replaceCard(text,t.cut,t.add));const legal=validateCommanderDeck(d.parsed,d.cards).isLegal,ff=d.cards.every(c=>printingMatchesPolicyV08(c,policy,AS_OF));const m=buildDeckMetrics(d.parsed,d.cards),y=ystolaOfferCount(d);const md=mean(SCENARIOS.map((s,i)=>delta(baseSims[i]!,signal(sim(d,s)))));const pass=legal&&ff&&comboLocked(d)&&d.parsed.totalCards===100&&m.landCount===m0.landCount&&m.rampCount>=m0.rampCount-1&&m.protectionCount>=m0.protectionCount-1&&md.keep>=-1.5&&md.uptime>=-4&&md.protection>=-7&&md.spells>=-0.22&&md.draws>=-0.32&&y>=y0-1;results.push({id:t.id,cut:t.cut,add:t.add,manualClass:t.manualClass,pass,meanDelta:md,ystolaOfferDelta:y-y0,metricsDelta:{mv:m.averageNonlandManaValue-m0.averageNonlandManaValue,ramp:m.rampCount-m0.rampCount,interaction:m.interactionCount-m0.interactionCount,protection:m.protectionCount-m0.protectionCount,early:m.earlyPlayCount-m0.earlyPlayCount}});}
