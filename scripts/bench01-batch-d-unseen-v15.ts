...
async function runFixture(fixture: typeof FIXTURES[number]): Promise<Json> {
  ...
  const client = new Client(
    { name: `bench01-batch-d-${fixture.id}`, version: '1.0.0' },
    { versionNegotiation: { mode: 'auto' } },
  );
  const transport = new StreamableHTTPClientTransport(new URL(`http://bench01-batch-d-${fixture.id}.local/mcp`), {
  ...
  return {
    fixture: `BENCH-01 Batch D / ${fixture.reference}`,
...
async function main(): Promise<void> {
  await unlink('bench01-batch-d-result.json').catch(() => undefined);
  const results: Json[] = [];
  for (const fixture of FIXTURES) results.push(await runFixture(fixture));
  const output = {
    schema: 'bench01-batch-d-unseen-v1',
    batch: 'BENCH-01-D',
    productRuntimeBaselineSha: FROZEN_PRODUCT_SHA,
    sourceFrozenWithinBatch: true,
    noCommanderIntelligenceChangesBetweenFixtures: true,
    purpose: 'Broaden adversarial evidence after corrected Batch C. Distinguish legitimate identity or construction ceilings from repeated inappropriate acceptance vetoes before authorizing any generic guard repair.',
    results,
  };
  await writeFile('bench01-batch-d-result.json', `${JSON.stringify(output, null, 2)}\n`);
  console.log(JSON.stringify(output, null, 2));
}

main().catch(async (error) => {
  await writeFile('bench01-batch-d-failure.txt', `${error instanceof Error ? error.stack ?? error.message : String(error)}\n`).catch(() => undefined);
  throw error;
});
