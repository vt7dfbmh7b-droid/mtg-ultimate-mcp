...
async function main(): Promise<void> {
  console.log('COUNTER BLITZ A17 BEST-SLOT PRESSURE FROM A16 CHAMPION');
  ...
  const baseline = await resolveDeck(renderChange(a14.parsed, 'From Father to Son', need('Commune with Beavers')));
  ...
  const pressureByName = new Map<string, (typeof fullAudit.slots)[number]>();
  ...
  for (const cutFinding of pressureCuts) {
    ...
    for (const add of candidates) {
      ...
      const mvDelta = cutCard.cmc - add.cmc;

      if (cutFinding.status === 'supported' && continuity === 0 && accessDelta < 1.5) continue;
      if (purposeDelta < 0 && accessDelta <= 0) continue;
      if (purposeDelta === 0 && accessDelta === 0 && continuity === 0) continue;

      const statusPressure = cutFinding.status === 'review' ? 2 : 0;
      const preScore = Number((statusPressure + purposeDelta * 2 + accessDelta + continuity * 0.75 + Math.max(-2, Math.min(2, mvDelta * 0.4))).toFixed(3));
      structural.push({
        cut: cutCard.name,
        add: add.name,
        ...
        purposeDelta,
        continuity,
        accessBefore: baselineAccess.weightedScore,
        accessAfter: access.after.weightedScore,
        accessDelta,
        manaValueDelta: Number(mvDelta.toFixed(3)),
        preScore,
        metrics,
        decklist,
        parsed,
  ...
  structural.sort((a, b) => num(b.preScore) - num(a.preScore) || num(b.purposeDelta) - num(a.purposeDelta) || num(b.accessDelta) - num(a.accessDelta));
  ...
  for (const candidate of finalists) {
    ...
    const reviewEligible = safe
      && mean > 0.15
      && positiveSeeds >= 3
      && (num(candidate.purposeDelta) > 0 || num(candidate.accessDelta) > 0)
      && (num(candidate.continuity) > 0 || num(candidate.accessDelta) >= 1.5);
    const finalScore = Number((mean + num(candidate.purposeDelta) * 0.4 + num(candidate.accessDelta) * 0.3 + num(candidate.continuity) * 0.15).toFixed(3));
    evaluated.push({
      cut: candidate.cut,
      add: candidate.add,
      ...
      metrics: {
        ...
        genericTutors: metrics.tutorCount,
    ...
    console.log(`${candidate.cut} -> ${candidate.add}: pre=${candidate.preScore} purpose=${candidate.purposeDelta} continuity=${candidate.continuity} access=${candidate.accessDelta} sim=${mean.toFixed(3)} min=${minimum.toFixed(3)} positive=${positiveSeeds}/5 safe=${safe} review=${reviewEligible}`);
  }
  ...
  if (leader) await writeFile('counter-blitz-a17-leader.txt', `${String(leader.decklist)}\n`, 'utf8');

  const output = {
    schema: 'counter-blitz-a17-best-slot-pressure-v1',
    baseline: 'A16 exploratory champion',
    baselineSwap: 'From Father to Son -> Commune with Beavers',
    baselineAudit: {
    ...
    reviewLeader: leader ? {
      ...
      continuity: leader.continuity,
      accessDelta: leader.accessDelta,
      simulationMean: leader.simulationMean,
      simulationMin: leader.simulationMin,
      positiveSeeds: leader.positiveSeeds,
      finalScore: leader.finalScore,
      metrics: leader.metrics,
    } : null,
    evaluated: evaluated.map(({ decklist: _decklist, parsed: _parsed, cards: _cards, ...candidate }) => candidate),
    note: 'Exploratory only. A17 starts from the accepted A16 champion, automatically pressure-tests weak nonland slots against the full FF-legal Bant pool, preserves real combo access and structural floors, and requires manual Oracle/strategic review before any swap is accepted. No stable/current promotion and no PR #29 merge.',
  };
  await writeFile('counter-blitz-a17-result.json', `${JSON.stringify(output, null, 2)}\n`, 'utf8');
}
...
try {
  ...
  const diagnostic = {
    schema: 'counter-blitz-a17-diagnostic-v1',
    message: error instanceof Error ? error.message : String(error),
    stack: error instanceof Error ? error.stack ?? null : null,
  };
  await writeFile('counter-blitz-a17-diagnostic.json', `${JSON.stringify(diagnostic, null, 2)}\n`, 'utf8');
  console.error(error);
  process.exitCode = 1;
}
